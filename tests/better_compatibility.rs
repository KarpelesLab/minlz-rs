/// Test that encode_better produces identical output to Go's s2.EncodeBetter
///
/// This test ensures binary compatibility with the Go reference implementation
/// for the "better" compression mode. The Rust implementation is based on the Go
/// code, so outputs must be identical for the same input.

#[test]
fn test_encode_better_matches_go() {
    // Input test data
...
";
    ...
    // Expected output from Go's s2.EncodeBetter (270 bytes)
    // Input: 705 bytes, Output: 270 bytes
    const EXPECTED_GO_OUTPUT: &[u8] = &[
        ...
        0x43, 0x20, 0x61, 0x74, 0x69, 0x62, 0x69, 0x6c, 0x69, 0x74, 0x79, 0x25, 0x79, 0x01, 0x95,
        0x25, 0x77, 0x04, 0x53, 0x32, 0x35, 0x61, 0x20, 0x69, 0x6f, 0x6e, 0x20, 0x66, 0x6f, 0x72,
        0x6d, 0x61, 0x11, 0x39, 0x15, 0x00, 0x65, 0x08, 0x50, 0x65, 0x72, 0x05, 0x7d, 0x30, 0x6e,
        0x63, 0x65, 0x20, 0x62, 0x65, 0x6e, 0x63, 0x68, 0x6d, 0x61, 0x72, 0x6b, 0x01, 0xa7, 0x30,
        0x61, 0x6e, 0x64, 0x20, 0x6f, 0x70, 0x74, 0x69, 0x6d, 0x69, 0x7a, 0x61, 0x74, 0x01, 0xa4,
        0x18, 0x76, 0x65, 0x72, 0x69, 0x66, 0x69, 0x63, 0x05, 0x0d, 0x11, 0x38, 0x15, 0x00, 0x62,
    ];

    // Compress with Rust's encode_better
    let rust_output = minlz::encode_better(INPUT);
    ...
    assert_eq!(
        ...
        "Rust encode_better output must match Go s2.EncodeBetter output exactly"
    );
}
